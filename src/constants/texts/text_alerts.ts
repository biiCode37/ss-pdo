...
export const TEXT_ALERTS = {
  ...
  BUS_INPUT_MODAL: {
    ...
    DIFF_NORMAL: (diff: number) => `Selisih: +${diff} KM`,
    DIFF_NEGATIVE: (diff: number) => `⚠️ KM Akhir lebih kecil dari KM Awal (${diff} KM)`,
    DIFF_EXTREME: (diff: number, max: number) => `⚠️ Selisih sangat besar (+${diff} KM, maks ${max} KM)`,
    LIVE_TOA_S2_RESULT: (tot: number, s1: number, s2: number) => `TOA Shift 2: ${s2} Penumpang (${tot} - ${s1})`,
    LIVE_TOA_S2_INVALID: (tot: number, s1: number) => `⚠️ Total TOA (${tot}) lebih kecil dari Shift 1 (${s1})`,
    COPY_KM_AKHIR_S1_BTN: '📋 Salin KM Akhir S1',
    HEADER_MODE_SHIFT1: 'Input Shift 1 (Closing Siang)',
    HEADER_MODE_SHIFT2: 'Input Shift 2 (Closing Malam)',
    HEADER_MODE_KM_AWAL: 'Input KM Awal',
    META_PLACEHOLDERS: {
      TOA_S1: 'Contoh: 120',
    ...
    SUBTITLE: 'Input Data Operasional Armada',
    VALIDATION_HEADER: 'Periksa kembali input:',
    LABEL_HAS_MANUAL_S1: 'Ada Tiket Manual Shift 1',
    LABEL_HAS_MANUAL_S2: 'Ada Tiket Manual Shift 2',
    PLACEHOLDER_MANUAL_S1: 'Jumlah Tiket Manual S1',
    PLACEHOLDER_MANUAL_S2: 'Jumlah Tiket Manual S2',
    DISTANCE_LABEL_S1: 'Jarak Tempuh S1:',
    DISTANCE_LABEL_S2: 'Jarak Tempuh S2:',
    TOTAL_TRIP_LABEL: 'Total Ritase:',
    TRIP_UNIT: 'Rit',
    FLEET_QUICK_STATUS: 'Status Cepat Armada',
    NOTES_DETAIL_LABEL: 'Rincian Catatan / Keterangan',
    NOTES_DETAIL_PLACEHOLDER: 'Contoh: AP / AC Kurang Dingin, BA.02 Pramudi Meriang',
    MODAL_CLOSE_ARIA: 'Tutup modal input bus',
    ...
    REF_KM_AWAL_DYNAMIC: (km: string | number, dateLabel: string) =>
      `Acuan KM (${dateLabel}): ${km}`,
    PREFILL_FROM_YESTERDAY: (km: string | number) => `Kemarin: ${km}`,
    PREFILL_DYNAMIC: (km: string | number, dateLabel: string) => `${dateLabel}: ${km}`,
  },
  MODAL_COMMON: {
    UNDERSTAND: 'Mengerti',
    CONTINUE: 'Lanjutkan',
